    ...
    switch (message.type) {
        ...
        case UUMessageTypePicture:
        {
            ...
            if (message.from == 0) {
                ...
                [self.btnContent.backImageView sd_setImageWithURL:[NSURL URLWithString:message.pictureUrl]
                                             placeholderImage:[UIImage imageNamed:@"ImgLoading"]];
            }
...
//            voiceURL = [NSString stringWithFormat:@"%@%@",RESOURCE_URL_HOST,message.strVoice];
    ...
    if (message.from == UUMessageFromMe) {
        self.activityView.frame = CGRectMake(self.btnContent.frame.origin.x - 30, self.btnContent.frame.origin.y + self.btnContent.frame.size.height / 2 - 15, 30, 30);
        if (message.state == UUMessageSending) {
