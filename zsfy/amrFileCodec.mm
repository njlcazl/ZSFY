...
int DecodeAMRFileToWAVEFile(const char* pchAMRFileName, const char* pchWAVEFilename)
{
	...
	//返回整数值：当str1<str2时，返回值<0； 当str1=str2时，返回值=0； 当str1>str2时，返回值>0。
	// 检查amr文件头
    fread(magic, sizeof(char), strlen(AMR_MAGIC_NUMBER), fpamr);
    
    //安卓   num=6, header=, standard=#!AMR
    //苹果 2668515411.amr  num=6, header=#!AMRt , standard=#!AMR
    
 //   num=6, header=#!AMRw standard=#!AMR
    
    
    printf("num=%ld, header=%s, standard=%s", strlen(AMR_MAGIC_NUMBER), magic, AMR_MAGIC_NUMBER); // qzlei
	if (strncmp(magic, AMR_MAGIC_NUMBER, strlen(AMR_MAGIC_NUMBER)))//#!AMR\n
	{
		fclose(fpamr);
		return 0;
	}
	...
	NSLog(@"amrFileCodec:frame[%d帧]", nFrameCount);
