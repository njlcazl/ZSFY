...
@interface TrialOpen_ViewController ()<UIScrollViewDelegate,UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate,RETableViewManagerDelegate>
{
    UITableView *tableViewProfile;//法院概况
    NSArray *listProfile;//法院慨况的数组
    UITableView *tableViewCase;//参考案例
//    NSArray *listCase;//参考案例的数组
    int casePageNo; //案件页数
     int caseSumNo; //案件页数
...
@implementation TrialOpen_ViewController
...
-(void)initWithView
{
    ...
    NSArray *listBtn = [NSArray arrayWithObjects:@"案件查询",@"参考案例",@"法院概况", nil];
    ...
    myScrollView.contentSize = CGSizeMake(Screen_Width*3, Screen_Height-64-34);
    for (int i = 0; i<3; i++) {
        ...
        if (i == 2) {
            ...
            tableViewCase.hidden = YES;
            tableViewCase.footer = [MJRefreshBackNormalFooter footerWithRefreshingTarget:self refreshingAction:@selector(addMore)];
            //    myTableView.separatorStyle = UITableViewCellSeparatorStyleNone;
...
{
   casePageNo ++;
    if (casePageNo > caseSumNo) {
        [MBProgressHUD showError:@"没有更多数据了" toView:tableViewCase];
        [tableViewCase.footer endRefreshing];
    }else
    {
        [self getProfile:7];
    }
...
//        paras
        ...
        [FSHttpTool post:@"app/article!paginationAjax.action" params:paras success:^(id json) {
            ...
            [tableViewCase.footer endRefreshing];
            NSDictionary *result = (NSDictionary *)json;
            NSLog(@"参考案例下的列表信息%@",json);
            NSDictionary *attach = [result objectForKey:@"attach"];
            caseSumNo = [[attach objectForKey:@"pageCount"]intValue];
            
           NSArray *caseArr = [attach objectForKey:@"list"];
            for (int i = 0; i < caseArr.count; i ++) {
                [self.listCase addObject:caseArr[i]];
            }
...
#pragma mark设置每一个cell的行高
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if ([tableView isEqual:tableViewCase]) {
        NSDictionary *dic = nil;
        if(self.listCase.count > indexPath.row)
        {
            dic = [self.listCase objectAtIndex:indexPath.row];
        }
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if ([tableView isEqual:tableViewProfile]) {
        ...
        NSString *identifier = @"CaseCell";
        FYCaseCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
        if (cell == NULL) {
            cell = [[FYCaseCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
        }
        NSDictionary *dic = nil;
