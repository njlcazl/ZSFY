...
@interface HawkManyFormsUIView()<UITableViewDataSource,UITableViewDelegate, UITextFieldDelegate>{
...
@implementation HawkManyFormsUIView
...
- (void)createCaseCheckView:(UIView *)view
{
    ...
    pickerBGView  = [[UIView alloc] initWithFrame:CGRectMake(0, Screen_Height - 44, Screen_Width, 216 + 40)];
    [self addSubview:pickerBGView];
    
    UIView * line = [[UIView alloc] initWithFrame:CGRectMake(0, 0,Screen_Width,0.7)];
    line.backgroundColor = FYColor(173, 173, 173);
    [pickerBGView addSubview:line];
    
    UIButton * ok = [[UIButton alloc] initWithFrame:CGRectMake(Screen_Width - 80,13, 70, 14)];
    [ok setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
    [ok setTitle:@"确定" forState:UIControlStateNormal];

    [ok addTarget:self action:@selector(stadiumButtonActions) forControlEvents:UIControlEventTouchUpInside];
    [pickerBGView addSubview:ok];
    
    _datePicker1 = [[UIDatePicker alloc] init];
    
    [_datePicker1 setTimeZone:[NSTimeZone defaultTimeZone]];
    
    _datePicker1.frame = CGRectMake(0, 40, Screen_Width, 216);
    
    _datePicker1.datePickerMode = UIDatePickerModeDate;
    
    _datePicker1.maximumDate = [[NSDate alloc] init];
    
    [self.datePicker1 addTarget:self
                        action:@selector(chooseDate:)
              forControlEvents:UIControlEventValueChanged];
    [pickerBGView addSubview:self.datePicker1];
    
    self.datePicker1.backgroundColor = [UIColor lightGrayColor];
    
}

- (void)showDatePicker{
    if (isExpandDatePicker == YES) {
        isExpandDatePicker = NO;
        [self hideDatePicker];
    }
    else{
        isExpandDatePicker = YES;
        
        [UIView animateWithDuration:0.3 animations:^{
            
            ajCheackTableView.frame = CGRectMake(ajCheackTableView.frame.origin.x, ajCheackTableView.frame.origin.y - 216 - 20, Screen_Width - 20, ajCheackTableView.height);
            
            pickerBGView.frame = CGRectMake(0, Screen_Height - 216 - 44 - 40, Screen_Width, 216 + 40);
...
- (void)hideDatePicker{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ajCheackTableView.frame = CGRectMake(ajCheackTableView.frame.origin.x, ajCheackTableView.frame.origin.y + 216 + 20, Screen_Width - 20, ajCheackTableView.height);
        pickerBGView.frame = CGRectMake(0, Screen_Height - 44, Screen_Width, 216 + 40);
        
    }];
}

- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event{
    if (![_datePicker1 isExclusiveTouch]) {
        [self showDatePicker];
    }
}
    ...
    if (indexPath.section == 0) {
        ...
        if (isExpandPicker) {
            ...
            else if (indexPath.row == 10)
            {
                ...
                __weak typeof (HawkManyFormsUIView *)__weakSelf = self;
                ...
                ajh1.text = @"日期*";
                ajh1.textAlignment = NSTextAlignmentLeft;
                ...
                [cell.contentView addSubview:ajh1];
                cell.detailTextLabel.text = self.currentDateString;
//                UILabel *ajh1 = [[UILabel alloc]initWithFrame:CGRectMake(5, 0,95 , 40)];
//                
//                ajh1.text = @"日期*";
//                
...
- (void)chooseDate:(UIDatePicker *)sender {
    ...
    self.currentDateString = [formatter stringFromDate:selectedDate];
}

- (void)stadiumButtonActions{
    NSIndexPath * indexRoload = nil;
    if (isExpandPicker) {
        indexRoload = [NSIndexPath indexPathForRow:11 inSection:1];
    }
    else{
        indexRoload = [NSIndexPath indexPathForRow:10 inSection:1];
    }
    [ajCheackTableView reloadRowsAtIndexPaths:[[NSArray alloc] initWithObjects:indexRoload, nil] withRowAnimation:UITableViewRowAnimationNone];
    [self hideDatePicker];
...
- (void)submit{
    ...
    params[@"movieDate"] = self.currentDateString;
